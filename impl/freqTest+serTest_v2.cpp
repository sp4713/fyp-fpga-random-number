...
void freqTest(ap_uint<32> freqStream[32], ap_uint<32> serialStream[128], ap_uint<32> seed, ap_uint<44> n) {
...
//frequency test parameters
	ap_uint<32> freqCount[32];		//frequency test fully partitioned array
//	#pragma HLS ARRAY_PARTITION variable=freqCount complete dim=1

//serial test parameters
	bool enableSerial = 0;			//prevents overlapping samples by toggling on every 2 samples
	ap_uint<32> seed_prev = 0;		//caches the previous seed value
	ap_uint<32> serialCount[128];	//serial test fully partitioned array
...
//	sampling	//
	...
	#pragma HLS LOOP_TRIPCOUNT min=100 max=1000
		...
		//seed_00 = ~(seed|seed_prev); 	//check for 00
		//seed_01 = (~seed) & seed_prev;	//check for 01
		//seed_10 = seed & (~seed_prev);	//check for 10
		//seed_11 = seed & seed_prev;		//check for 11
